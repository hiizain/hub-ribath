...
<div class="container my-5 pt-5">
    <div class="shadow rounded-5 p-5 bg-white">
        {{-- <h2>Daftar Program</h2>
        <hr> --}}
        <form id="daftarProgram">
            @csrf
            <div class="">
            ...
            <div class="form-group mb-3">
                ...
                <div class="row">
                    <div class="col-12">
                        <h5>Nama Program</h5>
                        <input class="form-control" type="text"value="{{ $program->nama_program.' '.$program->tahun }}">
                    </div>
                </div>
            </div>
            <div class="form-group mb-3">
                <h5>Deskripsi</h5>
                <textarea class="form-control">{{ $program->deskripsi }}</textarea>
                                ...
                                <div class="timeline">
                                    <div class="timeline__wrap">
                                        <div class="timeline__items">
                                            <?php $first = false; $active = false; $i=0; $count=count($tahap); ?>
            ...
            <div class="row">
                <div class="col-3">
                    <div class="form-group mb-3">
                        <h5>NIK</h5>
                        <input required maxlength="16" class="form-control" type="text" name="nik" placeholder="Masukkan NIK" value="{{ $santri->nik }}">
                    </div>
                </div>
                <div class="col-5">
                    <div class="form-group mb-3">
                        <h5>Nama</h5>
                        <input required class="form-control" type="text" name="nama" placeholder="Masukkan Nama Lengkap" value="{{ $santri->nama }}">
                    </div>
                </div>
                <div class="col-4">
                    <div class="form-group mb-3">
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="form-check">
                                    <input required class="form-check-input" type="radio" name="jenis_kelamin" id="laki" value="1" @if($santri->jenis_kelamin == '1') checked @endif>
                            ...
                            <div class="col-6">
                                <div class="form-check">
                                    <input required class="form-check-input" type="radio" name="jenis_kelamin" id="perempuan" value="0" @if($santri->jenis_kelamin == '0') checked @endif>
            ...
            <div class="row">
                <div class="col-6">
                    <div class="form-group mb-3">
                        <h5>Tempat Lahir</h5>
                        <input required class="form-control" type="text" name="tempat_lahir" placeholder="Tempat Lahir" value="{{ $santri->tempat_lahir }}">
                    </div>
                </div>
                <div class="col-6">
                    <div class="form-group mb-3">
                        <h5>Tanggal Lahir</h5>
                        <input required class="form-control" type="date" name="tanggal_lahir" placeholder="Pilih Tanggal Lahir" value="{{ $santri->tanggal_lahir }}">
                    </div>
                </div>
            </div>
            <div class="form-group mb-3">
                <h5>Alamat</h5>
                <textarea class="form-control" name="alamat" placeholder="Masukkan Alamat">{{ $santri->alamat }}</textarea>
                {{-- <input required class="form-control" type="text" value="{{ $santri->alamat }}"> --}}
            </div>
            <div class="row">
                <div class="col-6">
                    <div class="form-group mb-3">
                        <h5>Nama Wali</h5>
                        <input required class="form-control" type="text" name="nama_wali" placeholder="Masukkan Nama Wali" value="{{ $santri->nama_wali }}">
                    </div>
                </div>
                <div class="col-6">
                    <div class="form-group mb-3">
                        <h5>No. HP Wali</h5>
                        <input required maxlength="13" class="form-control" type="number" name="no_hp_wali" placeholder="Masukkan No. HP Wali" value="{{ $santri->no_hp_wali }}">
                    </div>
                </div>
            </div>
            <div class="form-group d-flex justify-content-end mb-3">
                <input type="hidden" name="program_id" value="{{ $program->id }}">
                <input type="hidden" name="user_id" value="{{ $santri->user_id }}">
                <a class="btn btn-danger" href="{{ route('program.index') }}">Batal</a>
                <button class="btn btn-success ms-2" type="submit">Daftar</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    const dataSantri = '<?php echo json_encode($santri) ?>';
    const santri = JSON.parse(dataSantri);
            ...
            function (data, textStatus, jqXHR) {
                if (data != null) {
                    if (data.data != null) {
                        ...
                        list = `<div class="col-6"><ul>`;
                        for(i=0; i<Math.ceil(count/2); i++){
                            list += `
                                <li>
                                    <p>`+data.data[i].nama_kegiatan+`</p>
                                </li>
                            `
                        }
                        list += `</ul></div><div class="col-6"><ul>`
    ...
    $('#daftarProgram').submit(function (e) {
        e.preventDefault();
        Swal.fire({
            icon: "question",
            title: "Peringatan",
            ...
            if (result.value) {
                $.ajax({
                    ...
                    // },
                    url: `${site_url}/program/daftar`,
                    // data: $('#detailProgram').serialize(),
                    // data: {

                    // },
                    data: new FormData($('#daftarProgram')[0]),
                    contentType: false,
                    processData: false,
                    dataType: 'json',
                    // beforeSend: function () { },
                    success: function (response) {
                        ...
                        if (response.status == 'success') {
                            Swal.fire('Update Berhasil!', response.message, 'success').then(function () {
                                location.href = site_url+'/user/program';
                            })
                        }
                    },
